...
public class bow4 extends BowItem {
    public bow4(Settings settings) {
        super(settings.maxDamage(6384));
    }
            ...
            if (!itemStack.isEmpty() || bl) {
                ...
                if ((double)f >= 0.3D) {
                    ...
                    if (!world.isClient) {
                        ...
                        persistentProjectileEntity.setProperties(playerEntity, playerEntity.pitch, playerEntity.yaw, 4.0F, f * 15.0F, 1.5F);
                        if (f == 3.0F) {
                    ...
                    world.playSound((PlayerEntity)null, playerEntity.getX(), playerEntity.getY(), playerEntity.getZ(), SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 7.0F, 7.0F / (RANDOM.nextFloat() * 6.4F + 9.2F) + f * 6.5F);
            ...
            if (!itemStack.isEmpty() || bl) {
                ...
                if ((double)f >= 0.3D) {
                    ...
                    if (!world.isClient) {
                        ArrowItem arrowItem = (ArrowItem)((ArrowItem)(itemStack.getItem() instanceof ArrowItem ? itemStack.getItem() : Items.ARROW));
                        PersistentProjectileEntity persistentProjectileEntity = arrowItem.createArrow(world, itemStack, playerEntity);
                        persistentProjectileEntity.setProperties(playerEntity, playerEntity.pitch, playerEntity.yaw, 4.0F, f * 15.0F, 0.0F);
                        if (f == 3.0F) {
                            persistentProjectileEntity.setCritical(true);
                        }
                        stack.damage(2, (LivingEntity)playerEntity, (Consumer)((p) -> {
